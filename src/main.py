...
from utils.enum import SmStateStatus
...
class Timer:
    ...
    async def start_timer(self):
        while True:
            if (sm.state == SmStateStatus.ARMED):
...
class Esc:
    def __init__(self):    
        self.pwm = pwmio.PWMOut(board.GP0, frequency=constant.pwm_frequency, duty_cycle=0)
        #Set 0% Power
...
class StateMachine:
    ...
    def transition(self, event):
        ...
        elif self.state == SmStateStatus.ARMED and event == "ARMED_TO_DISARMED":
            self.state = SmStateStatus.DISARMED
    ...
    def display_state(self):
        if self.state == SmStateStatus.ARMED:
            self.update_screen(20, 0xFF0000, "ACTIF")
        elif self.state == SmStateStatus.DISARMED:
            self.update_screen(0, 0x00FF00, "INACTIF")
        else:
            self.update_screen(0, 0xFFFFFF, "UNKNOWN")

    def update_screen(self, arg0, arg1, arg2):
        screen.text_area.x = arg0
        screen.text_area.color = arg1
        screen.text_area.text = arg2
